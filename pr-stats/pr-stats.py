...
class PRStat:
    ...
    def __init__(self):
        ...
        parser = argparse.ArgumentParser(description='GitHub PR statistics utill.', 
            usage="pr-stats.py [-h] [-v] [OPTIONS] [-(d|D) YYYY-MM-DD] <user> <repo>")

        parser.add_argument("-v", "--version", action='version', version='pr-stats v0.1 alpha', help="Print version")
        
        parser.add_argument("-o", "--days-opened", action="store_true", help="Print number of days PRs were open")
        parser.add_argument("-r", "--ratio", action="store_true", help="Print merged/closed ratio")
        parser.add_argument("-c", "--comments-number", action="store_true", help="Print number of comments")
        parser.add_argument("--day-of-week-opened", action="store_true", help="Show days of week when PRs were opened")
        parser.add_argument("--day-of-week-closed", action="store_true", help="Show days of week when PRs were closed")
        parser.add_argument("--hour-opened", action="store_true", help="Show hours of the day when PRs were opened")
        parser.add_argument("--hour-closed", action="store_true", help="Show hours of the day when PRs were closed")
        parser.add_argument("--week-opened", action="store_true", help="Show week when PRs were opened")
        parser.add_argument("--week-closed", action="store_true", help="Show week when PRs were closed")
        parser.add_argument("-u", "--user-opened", action="store_true", help="Print users who opened PRs")
        parser.add_argument("-U", "--user-closed", action="store_true", help="Print users who closed PRs")
        parser.add_argument("--labels", action="store_true", help="Print labels associated with PRs")
        parser.add_argument("-l", "--lines-added", action="store_true", help="Print number of lines added")
        parser.add_argument("-L", "--lines-deleted", action="store_true", help="Print number of lines deleted")
        
        parser.add_argument("-d", "--opened-before", type=self.__valid_date, help="Print PRs opened only BEFORE specified date")
        parser.add_argument("-D", "--opened-after", type=self.__valid_date, help="Print PRs opened only AFTER specified date")


        parser.add_argument("user", metavar="user", type=str, nargs=1, help='GitHub repo owner')
        parser.add_argument("repo", metavar="repo", type=str, nargs=1, help='GitHub repo name')    
    ...
    def __generate_request(self):
        ...
        self.__url = base_url + self.__args.user[0] + "/" + self.__args.repo[0] + "/pulls?state=all"
        return self.__url
    ...
    def __check_date(self, date):
        if self.__args.opened_before == None:
